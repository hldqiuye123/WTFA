...
class FrequencyRepresentationModule_TFA_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        # ori_xreal_1, ori_ximag_1 = torch.chunk(x, 2, 1)
        # ori_xreal_1 = ori_xreal_1[0,:,:].reshape(-1).numpy()
        # ori_ximag_1 = ori_ximag_1[0,:,:].reshape(-1).numpy()
        ...
        xreal_1 = xreal_1[0, :, :].reshape(-1).numpy()
        ximag_1 = ximag_1[0, :, :].reshape(-1).numpy()

        ximag_1 = hilbert(xreal_1).real
        ximag_1 = torch.tensor(ximag_1).float()
        ...
        ximag_2 = hilbert(xreal_2).real
        ximag_2 = torch.tensor(ximag_2).float()
        ...
        ximag_3 = hilbert(xreal_3).real
        ximag_3 = torch.tensor(ximag_3).float()
        ...
        ximag_4 = hilbert(xreal_4).real
        ximag_4 = torch.tensor(ximag_4).float()
